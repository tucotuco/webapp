      ...
      setup: function (cb) {
        ...
        if (this.pubList.isEmpty()) {
          map.init(_.bind(function() {
            ...
            var query = "SELECT orgname,icode,concat(orgcountry, ' ', orgstateprovince, ' ', orgcity) AS location,sum(count) AS records,sum(collectioncount) AS collections,count(title) AS resources FROM resource WHERE ipt=True and networks ='Amazonia' GROUP BY orgname, icode, location ORDER BY orgname";
