...
# Get actual stats from query_log_amazonia
def getExplicitStuff(tag):
    url = "https://vertnet.cartodb.com/api/v2/sql?api_key={2}&q=select+query,%20sum%28count%29+from+query_log_amazonia+where+query+like+%27%25{0}%3A%25%27%20and%20created_at%3E=date%20%27{1}%27%20group%20by%20query".format(tag, query_date_limit, api_key)
...
def getDownloadsData():
    # Monthly
    url = "https://vertnet.cartodb.com/api/v2/sql?api_key={1}&q=select%20concat%28year,%27-%27,month%29%20as%20date,%20queries,%20records%20from%20%28%20select%20extract%28month%20from%20date%28created_at%29%29%20as%20month,%20extract%28year%20from%20date%28created_at%29%29%20as%20year,%20count%28*%29%20as%20queries,%20sum%28count%29%20as%20records%20from%20query_log_amazonia%20where%20type=%27download%27%20and%20created_at%3E=date%20%27{0}%27%20group%20by%20extract%28month%20from%20date%28created_at%29%29,%20extract%28year%20from%20date%28created_at%29%29%20order%20by%20extract%28year%20from%20date%28created_at%29%29,%20extract%28month%20from%20date%28created_at%29%29%29%20as%20foo".format(query_date_limit, api_key)
    # Daily
    #url = "https://vertnet.cartodb.com/api/v2/sql?api_key={1}q=select%20date%28created_at%29,%20count%28*%29%20as%20queries,%20sum%28count%29%20as%20records%20from%20query_log_amazonia%20where%20type=%27download%27%20and%20created_at%3E=date%20%27{0}%27%20group%20by%20date%28created_at%29%20order%20by%20date%28created_at%29".format(query_date_limit, api_key)
    logging.info('QUERY URL: %s' % url)
    d = json.loads(urllib2.urlopen(url).read())['rows']
    downloadsdata = []
    for i in d:
...
def getMetadata():
    url = "https://vertnet.cartodb.com/api/v2/sql?api_key={1}&q=select%20type,%20count%28*%29%20as%20searches,%20sum%28count%29%20as%20records%20from%20query_log_amazonia%20where%20created_at%3E=date%20%27{0}%27%20group%20by%20type".format(query_date_limit, api_key)
    logging.info('QUERY URL: %s' % url)
    d = json.loads(urllib2.urlopen(url).read())['rows']
    metadata = {'query':{'searches':0, 'records':0}, 'download':{'searches':0, 'records':0}}
    for i in d:
        t = i['type']
        searches = i['searches']
        records = i['records']
...
def getRecordsQueried():
    url = "https://vertnet.cartodb.com/api/v2/sql?api_key={1}&q=select%20results_by_resource%20from%20query_log_amazonia%20where%20client=%27portal-prod%27%20and%20results_by_resource%20is%20not%20null%20and%20created_at%3E=date%20%27{0}%27%20and%20results_by_resource%20%3C%3E%20%27%7B%7D%27".format(query_date_limit, api_key)
    logging.info('QUERY URL: %s' % url)
    d = json.loads(urllib2.urlopen(url).read())['rows']
...
def getMaxDate():
    url = "https://vertnet.cartodb.com/api/v2/sql?api_key={0}&q=select%20max%28created_at%29%20from%20query_log_amazonia".format(api_key)
    logging.info('QUERY URL: %s' % url)
...
def insertDataCartoDB(mindate, searches, records_viewed, records_downloaded, downloads, download_data, class_data, institution_data):
    ...
    q = "insert into daily_amazonia_stats (mindate, searches, records_viewed, records_downloaded, downloads, download_data, class_data, institution_data) values ({0})".format(record)
